function [ Tau ] = Double_cloison_Tau_Num_AM_MatTra(h1,h2,E1,E2,f,k,theta,rho1,rho2,nu1,nu2,e,e2,kf,Zf,Zc1,Zc2,eta)
...
Tf1 = [cos(kx*e2) 1j.*Zf./cos(thetaf).*sin(kx.*e2); 1j.*cos(thetaf)./Zf.*sin(kx.*e2) cos(kx*e2) ];
...
for ii = 1:length(f)
    T(:,:,ii)  =  Tf1(:,:,ii)*Tpl1(:,:,ii)*Tf(:,:,ii)*Tpl2(:,:,ii);
end
%% Tau
