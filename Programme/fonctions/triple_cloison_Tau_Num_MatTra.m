function [ Tau ] = triple_cloison_Tau_Num_MatTra(h1,h2,h3,E1,E2,E3,f,k,theta,rho1,rho2,rho3,nu1,nu2,nu3,e,e2,kf,kf2,Zf,Zf2,Zc1,Zc2,eta)
...
kx2 = kf2.*cos(thetaf2);
Tf2 = [cos(kx2*e2) 1j.*Zf2./cos(thetaf2).*sin(kx2.*e2); 1j.*cos(thetaf2)./Zf.*sin(kx2.*e2) cos(kx2*e2) ];
...
for ii = 1:length(f)
    T(:,:,ii)  =  Tpl1(:,:,ii)*Tf(:,:,ii)*Tpl2(:,:,ii)*Tf2(:,:,ii)*Tpl3(:,:,ii);
end
%% Tau
